...
def parse_args():
	...
	parser.add_argument("-testmode", action="store_true", help="Open the first upcoming event immediately for testing")
	parser.add_argument("--jsconsole", action="store_true", help="Open the webview JS console")
...
def build_event_list_html(events):
	...
	for event in events:
		html += f"<li><b>{event.summary}</b><br>Start: {event.start}<br>End: {event.end}<br>Location: {event.location}<br>Module: {event.module_code}</li><br>"
	html += """
	</ul>
...
def open_lecture_webview(event, module_override=None, location_override=None):
	...
	def on_loaded():
		try:
			...
			if url and url.startswith(LECTURE_URL):
				...
				start_time_value = event.start.strftime('%H:%M')
				js = f'''
					(async function() {{
							...
							function waitForQrCell(timeout=10000) {{
								return new Promise((resolve, reject) => {{
									...
									function check() {{
										...
										if (targetRow) {{
											var qrCell = targetRow.querySelector('td[aria-label="QR code"]');
											if (qrCell) {{
				...
				print("Not on lectures or angular page yet; waiting for navigation...")
		except Exception as e:
	...
	def check_events():
		...
		try:
			...
			if testmode and events and not testmode_used:
				print("Test mode: opening first event immediately.")
				testmode_used = True
				open_lecture_webview(events[0], module_override="COMP/3007/01/SPR", location_override="JC-EXCHANGE-C33")
			else:
				while events and events[0].start <= now:
					event = events.pop(0)
